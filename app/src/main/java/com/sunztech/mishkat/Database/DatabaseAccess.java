...
public class DatabaseAccess {
    ...
    public ArrayList<HadithItem> getChapterHadith(int chapterId) {
        ...
        c = db.rawQuery("select * from tbl_Mishkat WHERE Kitab_ID = " + chapterId, new String[]{});
    ...
    public void updateBookMark(HadithItem hadithItem)
    {
        ...
        db.update("tbl_Mishkat", args, "hadees_number = " + hadithItem.getHadees_number(), null);
    }

    public ArrayList<HadithItem> getBookMarks()
    {
        HadithItem hadees = null;
        c = db.rawQuery("select * from tbl_Mishkat WHERE is_bookmarked = " + 1, new String[]{});
        ArrayList<HadithItem> hadeesItems = new ArrayList<>();
        if (c != null) {
            while (c.moveToNext()) {
    ...
    public ArrayList<HadithItem> searchHadith(String columnName, String searchingValue) {
        ...
        if(columnName.equals("hadees_number")) {
            c = db.rawQuery("select * from tbl_Mishkat WHERE " + columnName + " = " + searchingValue, new String[]{});

        }
        else{
            c = db.rawQuery("select * from tbl_Mishkat WHERE "  + columnName + " like '%" + searchingValue + "%'", new String[]{});
